...
/**
 * RBTree:红黑树  # 研究红黑树时叶子节点指的是最后的空节点(我们原来理解的叶子节点的孩子节点)
 * 1.每个节点都是有颜色的，要么是黑色要么是红色；
 * 2.根节点是黑色；
 * 3.每个叶子节点都是黑色的空节点（NIL），也就是说，叶子节点不存储数据；
 * 4.任何相邻的节点都不能同时为红色，也就是说，红色节点是被黑色节点隔开的；另一种表述：如果一个节点是红色的，那么它的孩子节点都是黑色的
 ...
 * 而红黑树允许出现4节点(包含3个值的节点是4节点，所谓的2、3、4是指该节点可以有几条伸出的边，即可以有几个孩子)，红黑树没有限制只有左连接可以是红链接。
 ...
 * 将put方法中的flipColors()语句(连同if判断)那一行，移动到递归调用之前(null判断和比较操作之间)
